...
class ResponsesTest extends AbstractKleinTest
{
    ...
    public function testFileSend()
    {
        ...
        $this->assertEquals(
            $file_mime,
            $this->router->response()->headers()->get('Content-Type')
        );
        $this->assertEquals(
            filesize(__FILE__),
            $this->router->response()->headers()->get('Content-Length')
        );
        $this->assertContains(
            $file_name,
            $this->router->response()->headers()->get('Content-Disposition')
        );
    }
    ...
    public function testJSON()
    {
        ...
        $this->assertEquals(
            'no-cache',
            $this->router->response()->headers()->get('Pragma')
        );
        $this->assertEquals(
            'no-store, no-cache',
            $this->router->response()->headers()->get('Cache-Control')
        );
        $this->assertEquals(
            'application/json',
            $this->router->response()->headers()->get('Content-Type')
